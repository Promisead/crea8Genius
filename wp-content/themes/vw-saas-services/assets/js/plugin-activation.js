jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_saas_services_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-saas-services-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-saas-services-plugin-install');
                button.addClass('button-primary vw-saas-services-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-saas-services-recommended-plugins .vw-saas-services-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-saas-services-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-saas-services-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-saas-services-recommended-plugins').css('display','none');
    });
